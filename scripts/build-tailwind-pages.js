...
const scssFiles = [
  path.resolve(__dirname, '../assets/scss/pages/index.scss'),
  path.resolve(__dirname, '../assets/scss/index.scss'),
];
const outputCssPath = path.resolve(__dirname, '../assets/css/index.css');

try {
  // Concatenate both SCSS files into a temp string
  const combinedScss = scssFiles.map(file => fs.readFileSync(file, 'utf8')).join('\n');
  // Compile SCSS to CSS
  const result = sass.compileString(combinedScss, { style: 'expanded' });
  // Ensure output directory exists
  fs.mkdirSync(path.dirname(outputCssPath), { recursive: true });
  // Write the compiled CSS
  fs.writeFileSync(outputCssPath, result.css);
  console.log(`✅ Compiled SCSS to: ${outputCssPath}`);
} catch (err) {
